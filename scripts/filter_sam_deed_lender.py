...
COLLECTION_FILTERED_SAM= "test_filtered_lender_sam"
COLLECTION_FILTERED_DEED = "test_filtered_lender_deed"
...
deed_filter = {
    "$and": [
        ...
        {"ConcurrentTDLenderName": {"$ne": ""}},
        {"ProjectId": project_id}
    ]
    }
total_documents = collection_deed.count_documents(deed_filter)
...
while skip < total_documents:
    ...
    cursor = collection_deed.find(deed_filter).skip(skip).limit(batch_size)
...
sam_filter = {
    "$and": [
        {"LC_TransactionDateValidForCompany": "Y"},
        {"LC_PropertyResidentialStatus": 1},
        {"LenderNameBeneficiary": {"$ne": ""}},
        {"ProjectId": project_id}
    ]
    }
total_documents = collection_sam.count_documents(sam_filter)
...
while skip < total_documents:
    ...
    # Fetch documents in batches
    cursor = collection_sam.find(sam_filter).skip(skip).limit(batch_size)
